...
function observeEvent(object, eventName, handler) {
    ...
    if (object != null) {
        if (object.addEventListener) {
            ...
            alert("opcion 1 observeEvent - object.addEventListener");
        } else if (object.attachEvent) {
            object.attachEvent("on" + eventName, //handler
                function (e) {return invokeAsMethod(object, handler,[e]);}
                //function(){return invokeAsMethod(object,handler,[window.event])}
            );
            /*
            switch(eventName){
                case 'keypress':
                    object.attachEvent("on" + eventName,function (e){ 
                                                                    return invokeAsMethod(object, handler, [e])
                                                                   }
                            );
                    break;
            ...
            */
            alert("opcion 2 observeEvent - object.attachEvent");
        } else {
            //alert("opcion 3 object[\"on\" + eventName] = handler");
            object["on" + eventName] = handler;
            
        }
       
    }else{
        alert("no se recupera el object");
    }
