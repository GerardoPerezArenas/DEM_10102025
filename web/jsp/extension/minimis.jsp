...
<script type="text/javascript">
    ...
    function recargarTablaMinimis(result) {
        try {
            if (!result || result.length === 0) {
                console.log('[Minimis] REFRESCO COMPLETO (legacy) resultado vac�o: sin cambios');
                return;
            }
            // Evitar refresco si la pesta�a no est� visible: bufferizamos para aplicar al activarse.
            if (typeof isMinimisTabVisible === 'function' && !isMinimisTabVisible()) {
                window._bufferMinimis = result;
                console.log('[Minimis] Refresco diferido (pesta�a oculta) - bufferizado filas=' + (result.length - 1));
                return;
            }

            // Serializar dataset nuevo para detectar cambios reales y evitar flicker por reconstrucci�n id�ntica.
            var nuevoHash = JSON.stringify(result);
            if (window._ultimoHashMinimis && window._ultimoHashMinimis === nuevoHash) {
                console.log('[Minimis] Datos id�nticos, se omite refresco.');
                return;
            }
    ...
    // Callback espec�fico para ventanas emergentes de Minimis.
    // Evita reutilizar retornoXanelaAuxiliar (usado por contrataciones) y que una pesta�a afecte a la otra.
    ...
    // Eliminado hook de interceptaci�n para volver a comportamiento estable.
...
<script  type="text/javascript">
    ...
    // Al hacer click en la pesta�a, si hay datos bufferizados los aplicamos.
    (function prepararActivacionPestania(){
        var cab = document.getElementById('pestana112');
        if(!cab) return;
        if (!cab._listenerMinimis){
            cab.addEventListener('click', function(){
                // Si la tabla aparece vac�a pero tenemos snapshot, restauramos.
                if (tablaMinimis && (!tablaMinimis.lineas || tablaMinimis.lineas.length===0) && window._snapshotMinimis && window._snapshotMinimis.length>0){
                    console.warn('[Minimis] Restaurando desde snapshot (tabla vac�a al activar)');
                ...
                if (window._bufferMinimis){
                    ...
                    console.log('[Minimis] Aplicando refresco diferido al activar pesta�a');
        ...
        var obs = new MutationObserver(function(mutations){
            ...
            if (tablaMinimis && window._snapshotMinimis && window._snapshotMinimis.length>0){
                if (cont.innerHTML.trim()===''){
                    ...
                    console.warn('[Minimis] Detectado contenedor vac�o, re-render desde snapshot | ' + causa.join(' | '));
                    if (!window._tablaMinimisInicializada){
                        inicializarTablaMinimis();
                        window._tablaMinimisInicializada = true;
                    }
